...
class DatabaseManager {
  ...
  async initialize() {
    ...
    if (this.config.useSocket) {
      ...
      try {
        ...
        const diagnosis = await this.diagnostics.diagnoseConnectionError(error, this.config);
  ...
  async _createSocketPool() {
    const socketConfig = {
      ...
      debug: this.config.debug === "true" ? ["ComQueryPacket", "RowDataPacket"] : false,
    };
    ...
    try {
      if (!this.pool) {
        return { 
          ...
          diagnostics: this.lastError ? await this.diagnostics.generateReport(this.lastError, this.config) : null
        };
      }
      ...
      try {
        ...
        return { 
          ...
          config: {
            ...
            socketPath: this.config.useSocket ? this.config.socketPath : undefined,
          }
        };
      } finally {
        connection.release();
      ...
      const diagnosis = await this.diagnostics.generateReport(error, this.config);
      return { 
        status: "unhealthy", 
        error: error.message,
        code: error.code,
