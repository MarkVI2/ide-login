...
class DiagnosticsManager {
  ...
  classifyError(error) {
    ...
    if (['ECONNREFUSED', 'ETIMEDOUT', 'ENOTFOUND'].includes(error.code)) return 'network';
    return 'unknown';
  }
  ...
  async getDatabaseInfo(config) {
    ...
    info.hostReachable = await this.checkHostReachability(config.host, config.port);
